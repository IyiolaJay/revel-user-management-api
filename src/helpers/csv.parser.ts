...
class CsvUploadHelper {
    ...
    static async validateCsvHeader(csv: Express.Multer.File, expectedHeaders: string[]): Promise<boolean> {
        try {
            const { path: filePath } = csv;
    ...
    static async getItemCategoryIdFromName(data: IItem[]) {
        const categoryRepository = new CategoryRepository();
      
        // Extract unique category names from the input data
        const _categoryNames = Array.from(new Set(data.map((item: any) => item.categoryName)));
      
        // Fetch matching categories using case-insensitive regex
        const _category = await categoryRepository.findAll(1, _categoryNames.length, {
          categoryName: {
        ...
        const categoryMap = new Map(
          _category.data.map((cat: any) => [cat.categoryName.toLowerCase(), cat._id.toString()])
        );
